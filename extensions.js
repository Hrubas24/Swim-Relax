...
const UPLOAD_PROXY_URL = "https://upload-proxy.matyashruby.workers.dev/"
  ...
  render: ({ element }) => {
    ...
    formContainer.innerHTML = `
      ...
      <input type="tel" class="phone" name="phone" required
             pattern="\\d+" title="Invalid phone number, please enter only numbers"><br><br>
...
export const VideoExtension = {
  name: "Video",
  type: "response",
  match: ({ trace }) => trace.type === "ext_video" || trace.payload?.name === "ext_video",
  render: ({ trace, element }) => {
...
export const TimerExtension = {
  name: "Timer",
  type: "response",
  match: ({ trace }) => trace.type === "ext_timer" || trace.payload?.name === "ext_timer",
  render: ({ trace, element }) => {
    const duration =
      trace?.payload && typeof trace.payload.duration === "number" ? trace.payload.duration : 5
    ...
    const countdown = setInterval(() => {
      if (timeLeft <= 0) {
        ...
        timeLeft -= 1
        timerContainer.querySelector("#time").textContent = String(timeLeft)
      }
    }, 1000)
...
export const FileUploadExtension = {
  ...
  render: ({ element }) => {
    ...
    fileInput.addEventListener("change", function () {
      ...
      fetch(UPLOAD_PROXY_URL, { method: "POST", body: data })
        .then(async (response) => {
...
export const KBUploadExtension = {
  name: "KBUpload",
  type: "response",
  match: ({ trace }) => trace.type === "ext_KBUpload" || trace.payload?.name === "ext_KBUpload",
  render: ({ trace, element }) => {
    const apiKey = trace.payload?.apiKey || null
    const maxChunkSize = trace.payload?.maxChunkSize || 1000
    const tags = trace.payload?.tags ? `tags=${JSON.stringify(trace.payload.tags)}&` : ""
...
export const DateExtension = {
  name: "Date",
  type: "response",
  match: ({ trace }) => trace.type === "ext_date" || trace.payload?.name === "ext_date",
  render: ({ element }) => {
    const formContainer = document.createElement("form")
    ...
    formContainer.innerHTML = `
      ...
      <div class="meeting">
        <input type="datetime-local" id="meeting" name="meeting" value=""
...
export const FeedbackExtension = {
  name: "Feedback",
  type: "response",
  match: ({ trace }) => trace.type === "ext_feedback" || trace.payload?.name === "ext_feedback",
  render: ({ element }) => {
